...
public class Level3Door : MonoBehaviour
{
	...
	public bool doorIsDestroyed = false;
	...
	void Awake ()
	{
		...
		explosionShotFX = GameObject.FindGameObjectWithTag("doorShotExplosion").GetComponent<ParticleSystem>();
		explosionSmokeFX = GameObject.FindGameObjectWithTag("doorSmokeExplosion").GetComponent<ParticleSystem>();
	...
	void OnTriggerEnter2D (Collider2D col) 
	{
		...
		if (col.tag == "shotgunBullet" || col.tag == "Bomb" && !doorIsDestroyed) {
			if (damageOne)
			{
